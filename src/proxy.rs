...
/// over the shorter is the worktree suffix (`yapcut-web` + variant
/// `yapcut-web-billing` -> `billing`), and any other route ending in
...
mod tests {
    ...
    fn related_routes_finds_worktree_variants() {
        let active = vec![
            route("yapcut-web-subscription-billing"),
            route("yapcut-webby"),
            route("zhcb-api"),
        ];
        let (suggested, same_wt, others) = related_routes("yapcut-web", &active);
        assert_eq!(names(&suggested), ["yapcut-web-subscription-billing"]);
        assert!(same_wt.is_empty());
        assert_eq!(names(&others), ["yapcut-webby", "zhcb-api"]);
    }
    ...
    fn related_routes_groups_same_worktree_siblings() {
        let active = vec![
            route("yapcut-web-subscription-billing"),
            route("yapcut-server-subscription-billing"),
            route("yapcut-studio-subscription-billing"),
            route("zhcb-api-audit-task"),
        ];
        let (suggested, same_wt, others) = related_routes("yapcut-web", &active);
        assert_eq!(names(&suggested), ["yapcut-web-subscription-billing"]);
        assert_eq!(
            names(&same_wt),
            [
                "yapcut-server-subscription-billing",
                "yapcut-studio-subscription-billing"
            ]
        );
        assert_eq!(names(&others), ["zhcb-api-audit-task"]);
    }

    #[test]
    fn related_routes_finds_base_for_stale_worktree_host() {
        let active = vec![route("yapcut-web"), route("zhcb-api")];
        let (suggested, same_wt, others) = related_routes("yapcut-web-old-branch", &active);
        assert_eq!(names(&suggested), ["yapcut-web"]);
        assert!(same_wt.is_empty());
        assert_eq!(names(&others), ["zhcb-api"]);
    }
    ...
    fn related_routes_empty_for_unrelated_label() {
        ...
        assert!(same_wt.is_empty());
        assert_eq!(names(&others), ["yapcut-web", "zhcb-api"]);
    }
    ...
    fn not_found_html_sections_related_routes() {
        ...
        let html = not_found_html("yapcut-web", "yapcut-web.localhost:1355", "http", &active);
        assert!(html.contains("Did you mean"));
        assert!(html.contains("http://yapcut-web-subscription-billing.localhost:1355"));
        assert!(html.contains("Same worktree"));
        assert!(html.contains("http://yapcut-server-subscription-billing.localhost:1355"));
        assert!(html.contains("Other running apps"));
        assert!(html.contains("zhcb-api"));
    }

    #[test]
    fn not_found_html_plain_list_when_nothing_related() {
        let html = not_found_html("zzz", "zzz.localhost:1355", "http", &[route("yapcut-web")]);
        assert!(html.contains("Active apps"));
        assert!(html.contains("yapcut-web"));
        assert!(!html.contains("Did you mean"));
    }
}
